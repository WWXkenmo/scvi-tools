...
class VAE(BaseLatentModeModuleClass):
    ...
    def __init__(
        ...
        self.n_input_feature = n_input_encoder
    ...
    def add_pseudoinputs(self,n_input1,n_input2,mean,var):
        if self.inject_covariates:
            ...
            self.means = FCLayers(self.number_vp_components, n_input1+n_input2,bias = False, use_batch_norm = True, dropout_rate = 0)
            self.means.fc_layers[0][0].weight.data.normal_(mean,var)
    ...
    def _regular_inference(
        ...
        if self.use_metaprior and self.use_vampprior:
    ...
    def generative(
        ...
        if self.use_vampprior:
            ...
            if self.inject_covariates:
                ...
                if self.n_batch == 1:
                    self.z_encoder.encoder.surgery_comp = torch.tensor([]).to(device)
    ...
    def vp_kl(self,inference_outputs, generative_outputs):
        ...
        if "mixture_weight" in list(inference_outputs.keys()):
            w = torch.add(inference_outputs["mixture_weight"],10e-6)
